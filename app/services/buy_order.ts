...
import { encodeFunctionData, formatUnits, parseAbi, parseAbiItem, parseUnits } from "viem";
...
export function useBuyOrderMutation() {
    ...
    return useMutation({
        mutationFn: async ({ assets, accountId, totalAmountToBeInvested }: CreateBuyOrderArgs) => {
            ...
            const orderEvents = await publicClient.getLogs({
                address: orderProcessorAddress,
                event: parseAbiItem('event OrderCreated(uint256 orderId, address account)'),
            ...
            for (const [index, orderEvent] of orderEvents.entries()) {
                const { args } = orderEvent;
                const [orderId, orderAccount] = args;
                const orderStatusCode = await publicClient.readContract({
                    address: orderProcessorAddress,
                    abi: orderProcessorAbi,
