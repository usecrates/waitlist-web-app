    ...
    <div className="flex flex-col items-center gap-2">
      {authenticated && address ? (
        <div className="flex gap-2 text-black">
          <button onClick={copyToClipboard} className="bg-black dark:bg-white font-ropa flex text-white px-4 py-2 font-medium text-md">
            <span>{shortAddress}</span> <Copy className="h-4 w-4 mx-2 mt-1" />
          </button>
          <button onClick={logout} className="bg-black dark:bg-white font-ropa text-white px-4 py-2 font-medium text-md">
            Logout
          </button>
        </div>
      ) : (
